...
public class Stage2 extends Scene implements ActionListener{
	...
	public Stage2() {
	    ...
	    //Disturbance arrow = new Arrow(0, 700, 0, 0, 0, 0, "https://i.imgur.com/Gm12azv.jpg");
	    for (int i = 0; i < 2; ++i) {
	...
	/*
	    ...
	    imagePanel.setFocusable(true);
	    this.imagePanel.addKeyListener(new KeyAdapter() {
	        public void keyPressed(KeyEvent e) {
	            if (isdropped == false){
	            	switch(e.getKeyCode()) {
	                case KeyEvent.VK_SPACE:
	                	//press SPACE to release person
	                    persons.get(0).lb.setVisible(true);
	                    persons.get(0).velocityX = 0.5;
	                    persons.get(0).velocityY = 1;
	                    persons.get(0).positionX = aircrafts.get(0).positionX;
	                    persons.get(0).positionY = aircrafts.get(0).positionY;
	                    persons.get(0).lb.setLocation((int)persons.get(0).getPositionX(), (int)persons.get(0).getPositionY());
	                    isdropped = true;
	                    break;
	               }
	            }
