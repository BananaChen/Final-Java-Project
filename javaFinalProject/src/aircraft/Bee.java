...
public class Bee extends Aircraft {
	...
	public Bee(double x, double y, double vx, double vy, double ax, double ay, String imagePath) {
		...
		/*
		 * try { ImageIcon icon = new ImageIcon(new URL(a));
		 * icon.setImage(icon.getImage().getScaledInstance(width,height,Image.
		 * SCALE_DEFAULT)); lb = new JLabel(icon); lb.setLocation(1,1);
		 * lb.setSize(width,height); } catch (MalformedURLException e) {
		 * e.printStackTrace(); }
		 */
